...
public class JWTokenProvider implements TokenProvider {
    ...
    protected final String generateJWToken(SignatureAlgorithm sig, Calendar timeToLive) {
        try {
            ...
            JwtBuilder builder = Jwts.builder()
                    .signWith(sig, key);
            //
            if(getHeader() != null) builder.setHeaderParams(getHeader().marshalling(true));
    ...
    private String getPayload() {
        if (this.payload != null){
            try {
                return MessageParser.getJsonSerializer().writeValueAsString(this.payload);
