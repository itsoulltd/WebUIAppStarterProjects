...
## Running the app in Docker: Create Docker Image for CICD deployment
    ...
    ~>$ docker build -t nginx-react-app-dev:1.0 .
    ~>$ docker run -d -p 80:80 -p 443:443 --name my-react-app nginx-react-app-dev
    
    #Create pro-image using Dockerfile-Pro:
    ~>$ docker build -f Dockerfile-Pro -t nginx-react-app-pro:1.0 .
    ~>$ docker run -d -p 80:80 -p 443:443 --name my-react-app nginx-react-app-pro
